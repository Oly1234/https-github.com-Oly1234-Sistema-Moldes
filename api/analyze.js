...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (action === 'FIND_WHITE_MODELS') {
        ...
        // Prompt para gerar 50+ queries altamente específicas
        const MOCKUP_PROMPT = `Generate 55 unique image search queries for a model wearing a SOLID PLAIN WHITE ${reference}. 
        CRITICAL: Focus on professional studio photography with HIGH CONTRAST backgrounds (dark grey, deep blue, textured wood, outdoor nature) to make the white garment pop. 
        Ensure variety in poses and lighting. 
        Output JSON ONLY: { "queries": ["query 1", "query 2", ...] }`;

        const endpoint = `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=${apiKey}`;
        const response = await fetch(endpoint, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ contents: [{ parts: [{ text: MOCKUP_PROMPT }] }], generationConfig: { response_mime_type: "application/json" } }) });
    ...
    // ... outros endpoints existentes (SCAN_CLOTHING, GENERATE_PATTERN, etc)
    
    // Fallback para Scan original se não houver action
