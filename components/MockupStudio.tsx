...
// --- ASSETS VIRTUAIS (CROQUI 2.5D) ---
const VIRTUAL_MODEL = {
    skinPath: "M180,60 Q200,50 220,60 L230,150 L240,400 L230,580 L170,580 L160,400 L170,150 Z", 
    parts: {
        FRENTE: {
            path: "M175,120 Q200,150 225,120 Q235,140 230,220 Q200,230 170,220 Q165,140 175,120",
            zIndex: 2,
            shadowMap: "linear-gradient(90deg, rgba(0,0,0,0.2) 0%, transparent 20%, transparent 80%, rgba(0,0,0,0.2) 100%)" 
        },
        COSTAS: {
            path: "M175,120 Q200,130 225,120 L230,220 Q200,210 170,220 Z", 
            zIndex: 0,
            shadowMap: "linear-gradient(90deg, rgba(0,0,0,0.3) 0%, transparent 50%, rgba(0,0,0,0.3) 100%)"
        },
        SAIA: {
            path: "M170,220 Q200,230 230,220 L245,500 Q200,520 155,500 Z",
            zIndex: 1,
            shadowMap: "repeating-linear-gradient(90deg, transparent, transparent 40px, rgba(0,0,0,0.1) 45px, transparent 50px)"
        },
        MANGA: {
            pathLeft: "M175,120 Q150,110 140,160 Q150,190 170,180",
            pathRight: "M225,120 Q250,110 260,160 Q250,190 230,180",
            zIndex: 3,
            shadowMap: "radial-gradient(circle at 50% 0%, transparent, rgba(0,0,0,0.3))"
        },
        GOLA: {
            path: "M175,120 Q200,150 225,120 Q200,130 175,120 Z",
            zIndex: 4,
            shadowMap: ""
        }
    }
...
const floodFill = (
    ...
    const r0 = data[startPos]; const g0 = data[startPos + 1]; const b0 = data[startPos + 2]; const a0 = data[startPos+3];
...
export const MockupStudio: React.FC = () => {
  // --- STATE ---
  const [tabMode, setTabMode] = useState<'TECHNICAL' | 'MOCKUP' | 'AI_CREATOR'>('TECHNICAL');
  ...
  const [generatedPatternUrl, setGeneratedPatternUrl] = useState<string | null>(null);
  ...
  const handleGeneratePattern = async () => {
      ...
      try {
          ...
          if (data.success && data.image) {
              setGeneratedPatternUrl(data.image);
          } else {
  ...
  const applyGeneratedPattern = () => {
      if (generatedPatternUrl) {
          setPatternImage(generatedPatternUrl);
          setTabMode('TECHNICAL'); // Volta para o editor para aplicar
    ...
    <div className="flex flex-col h-full bg-[#f0f2f5] md:flex-row overflow-hidden font-sans relative"
      ...
      <div className="w-full md:w-80 bg-white border-r border-gray-200 flex flex-col shadow-2xl z-20 shrink-0 h-auto md:h-full overflow-y-auto">
          ...
          <div className="flex border-b border-gray-200">
              ...
              <button onClick={() => setTabMode('AI_CREATOR')} className={`flex-1 py-3 text-xs font-bold flex items-center justify-center gap-1 ${tabMode==='AI_CREATOR' ? 'text-purple-600 border-b-2 border-purple-600' : 'text-gray-400'}`}><Sparkles size={12}/> AI STUDIO</button>
               ...
               {tabMode === 'AI_CREATOR' && (
                   <div className="animate-fade-in space-y-4">
                       <div className="bg-purple-50 border border-purple-200 rounded-xl p-4">
                           <h3 className="text-xs font-bold text-purple-800 mb-2 flex items-center gap-2"><Sparkles size={14}/> CRIADOR DE ESTAMPAS</h3>
                           <textarea 
                                value={aiPrompt}
                                onChange={(e) => setAiPrompt(e.target.value)}
                                placeholder="Descreva sua estampa (ex: Floral tropical estilo Farm Rio fundo escuro...)"
                                className="w-full h-24 p-3 rounded-lg border border-purple-200 text-sm focus:ring-2 focus:ring-purple-500 outline-none resize-none"
                           />
                           <button 
                                onClick={handleGeneratePattern}
                                disabled={isGenerating || !aiPrompt.trim()}
                                className="w-full mt-3 py-3 bg-gradient-to-r from-purple-600 to-indigo-600 text-white rounded-lg font-bold text-sm shadow-lg hover:opacity-90 disabled:opacity-50 flex items-center justify-center gap-2"
                           >
                               {isGenerating ? <Loader2 size={16} className="animate-spin"/> : <Brush size={16}/>}
                               {isGenerating ? 'CRIANDO TEXTURA...' : 'GERAR ESTAMPA'}
                           </button>
                       </div>

                       {generatedPatternUrl && (
                           <div className="space-y-2">
                               <div className="aspect-square w-full rounded-xl overflow-hidden border-2 border-purple-200 shadow-md relative group">
                                   <img src={generatedPatternUrl} className="w-full h-full object-cover"/>
                                   <div className="absolute inset-0 bg-black/50 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                                       <a href={generatedPatternUrl} download="estampa-vingi-ai.jpg" className="p-2 bg-white rounded-full text-black mx-1"><Download size={16}/></a>
                                   </div>
                               </div>
                               <button 
                                   onClick={applyGeneratedPattern}
                                   className="w-full py-3 bg-vingi-900 text-white rounded-lg font-bold text-sm shadow-xl flex items-center justify-center gap-2 animate-bounce-subtle"
                               >
                                   USAR NO MOLDE <ArrowRight size={16}/>
                               </button>
                           </div>
                       )}
                   </div>
      ...
      <div className="flex-1 bg-[#e2e8f0] relative flex items-center justify-center overflow-auto p-4">
           ...
           {tabMode === 'AI_CREATOR' && !generatedPatternUrl && (
               <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                   <div className="text-center opacity-40">
                       <Sparkles size={64} className="mx-auto mb-4 text-purple-500"/>
                       <h2 className="text-2xl font-bold text-slate-500">AI Pattern Studio</h2>
                       <p className="text-sm text-slate-400">Crie texturas exclusivas com Inteligência Artificial</p>
                   </div>
               </div>
           )}

           <div className={`relative shadow-2xl bg-white border-4 border-white rounded-lg flex flex-col items-center justify-center overflow-hidden transition-all duration-500 ${tabMode === 'AI_CREATOR' && generatedPatternUrl ? 'opacity-20 blur-sm scale-95' : 'opacity-100 scale-100'}`}>
                {moldImgObj ? (
                    <canvas 
                        ref={mainCanvasRef}
                        width={canvasDims.w}
