...
import { UploadCloud, Image as ImageIcon, RotateCw, ZoomIn, Eraser, Download, Wand2, MonitorPlay, X, Target, FlipHorizontal, Lock, Unlock, Layers, Zap, MousePointer2, CheckCircle2, Sparkles } from 'lucide-react';
...
export const MockupStudio: React.FC = () => {
  ...
  const [globalMirror, setGlobalMirror] = useState(false);
  ...
  useEffect(() => {
      if (simImage) { const img = new Image(); img.src = simImage; img.onload = () => setSimImgObj(img); }
  }, [simImage]);

  // Sync Settings
  useEffect(() => {
    if (syncSettings && layers.length > 0) {
        setLayers(prev => prev.map(layer => ({ ...layer, scale: globalScale, rotation: globalRotation, isMirrored: globalMirror })));
    }
  ...
  useEffect(() => {
      if (!syncSettings && activeLayerId) {
          ...
          if (layer) { setGlobalScale(layer.scale); setGlobalRotation(layer.rotation); setGlobalMirror(layer.isMirrored); }
      }
  ...
  const render = useCallback(() => {
    ...
    layers.forEach(layer => {
        ...
        if (layer.isMirrored) lCtx.scale(-1, 1);
        lCtx.translate(-layer.maskCenter.x, -layer.maskCenter.y);
        ...
        if (pat) { lCtx.fillStyle = pat; const safeSize = 20000; lCtx.fillRect(-safeSize/2, -safeSize/2, safeSize, safeSize); }
        lCtx.restore();
        ctx.save();
        ...
        if (layer.id === activeLayerId) {
            ...
            ctx.beginPath();
            ctx.arc(layer.maskCenter.x, layer.maskCenter.y, 8, 0, Math.PI * 2); // Bigger dot
  ...
  const renderSimulation = useCallback(() => {
      ...
      const ctx = canvas.getContext('2d')!;
      if (canvas.width !== 300) { const ratio = simImgObj.width / simImgObj.height; canvas.width = 300; canvas.height = 300 / ratio; }
      ctx.clearRect(0, 0, canvas.width, canvas.height);
      ...
      simMappings.forEach(mapping => {
          ...
          if (pat) { ctx.fillStyle = pat; ctx.fillRect(-500, -500, 1000, 1000); }
          ctx.restore();
      });
  }, [simImgObj, layers, simMappings, realismOpacity]);

  useEffect(() => { requestAnimationFrame(render); }, [render]);
  useEffect(() => { if(showSimModal) requestAnimationFrame(renderSimulation); }, [renderSimulation, showSimModal]);
  ...
  const createMaskFromClick = (startX: number, startY: number) => {
    ...
    if (data[startPos] < 50 && data[startPos + 1] < 50 && data[startPos + 2] < 50) return null;
    ...
    // Flood Fill (Optimized for brevity)
    while (stack.length) {
        const [x, y] = stack.pop()!;
        let py = y;
                    ...
                    if (res) {
                        ...
                        if (hasNewPixels) {
                            newLayers.push({
                                ...
                                scale: globalScale, rotation: globalRotation, isMirrored: globalMirror, timestamp: Date.now()
                            });
                        }
                    }
                }
  ...
  const handleCanvasClick = (e: React.MouseEvent | React.TouchEvent) => {
    ...
    if (tool === 'WAND') {
        ...
        if (res) {
            ...
            setTool('MOVE'); // Auto switch to move for better UX
        }
    } else if (tool === 'MOVE') {
        let clickedId = null;
  ...
  const handleMove = (e: React.MouseEvent | React.TouchEvent) => {
    ...
    // Não prevenir default se for apenas rolagem no mobile
    // Mas aqui estamos no canvas, então prevenimos
    ...
    if (layer) {
        ...
        if (layer.isMirrored) dxLocal = -dxLocal;
  ...
  return (
    <div className="flex flex-col md:flex-row h-full w-full bg-[#f0f2f5] overflow-hidden relative">
      ...
      <div className="order-2 md:order-1 w-full md:w-80 bg-white border-t md:border-t-0 md:border-r border-gray-200 flex flex-col shadow-2xl z-20 h-[50vh] md:h-full overflow-y-auto">
          ...
          <div className="p-5 space-y-6 pb-24 md:pb-5">
              ...
              <div className="space-y-3">
                  <div className="flex gap-2">
                      <button 
                        ...
                        className={`flex-1 py-3 rounded-xl border-2 text-xs font-bold flex flex-col items-center gap-1 transition-all ${tool === 'WAND' ? 'border-vingi-600 bg-vingi-50 text-vingi-700' : 'border-gray-100 bg-white text-gray-400'}`}
                      >
                          <Zap size={16}/> SELECIONAR
                      </button>
                      <button 
                        onClick={() => setTool('MOVE')} 
                        className={`flex-1 py-3 rounded-xl border-2 text-xs font-bold flex flex-col items-center gap-1 transition-all ${tool === 'MOVE' ? 'border-vingi-600 bg-vingi-50 text-vingi-700' : 'border-gray-100 bg-white text-gray-400'}`}
                      >
                          <MousePointer2 size={16}/> MOVER
                      </button>
                  </div>
              ...
              <div className={`bg-white rounded-xl border-2 transition-all duration-300 ${activeLayerId ? 'border-blue-200 shadow-lg' : 'border-gray-100 opacity-50 pointer-events-none'}`}>
                   <div className="p-3 bg-blue-50/50 border-b border-blue-100 flex justify-between items-center">
                       ...
                       <button onClick={() => setSyncSettings(!syncSettings)} className={`p-1 rounded ${syncSettings ? 'text-blue-600 bg-blue-100' : 'text-gray-400'}`}>
                   ...
                   <div className="p-4 space-y-4">
                       ...
                       <div className="grid grid-cols-2 gap-2">
                           <button 
                             ...
                             className={`py-2 rounded-lg border text-[10px] font-bold flex items-center justify-center gap-2 transition-colors ${globalMirror ? 'bg-vingi-900 text-white border-vingi-900' : 'bg-white text-gray-600 border-gray-200 hover:bg-gray-50'}`}
                           >
                               <FlipHorizontal size={12} /> {globalMirror ? 'ESPELHADO' : 'ESPELHAR'}
                           </button>
                           <button onClick={() => setLayers([])} className="py-2 bg-red-50 text-red-500 border border-red-100 rounded-lg text-[10px] font-bold flex items-center justify-center gap-2 hover:bg-red-100">
