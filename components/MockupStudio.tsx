  ...
  const handlePointerDown = (e: React.MouseEvent | React.TouchEvent) => {
      ...
      if (tool === 'WAND') {
          ...
          if (clickedId) {
              ...
              // Clicked on empty space (or mold background) -> Deselect current layer
              // This fixes the issue of "clica e não faz nada" when trying to select something else
          ...
          <div className="p-4 md:p-5 space-y-4 md:space-y-6 overflow-y-auto max-h-full">
               ...
               {activeLayer ? (
                   <div className="bg-blue-50 p-3 rounded-xl border border-blue-200 shadow-sm animate-fade-in-up">
                       ...
                       {/* BOTÃO DE NOVA APLICAÇÃO RÁPIDA */}
                       <button 
                            onClick={() => { setActiveLayerId(null); setTool('WAND'); }} 
                            className="w-full mb-2 bg-vingi-900 text-white p-3 rounded-lg flex items-center justify-center gap-2 font-bold text-xs shadow-lg active:scale-95"
                       >
