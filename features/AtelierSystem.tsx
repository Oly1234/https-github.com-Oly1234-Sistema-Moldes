...
import { UploadCloud, Wand2, Download, Palette, Loader2, Layers, Grid3X3, ArrowDownToLine, Check, Printer, Brush, Info, Settings2, Ruler, Scroll, Maximize, FileWarning, Zap, Grip, AlignVerticalSpaceAround, Spline, RefreshCw, Droplets, Sun, Moon, Contrast, Sparkles, X } from 'lucide-react';
...
export const AtelierSystem: React.FC<AtelierSystemProps> = ({ onNavigateToMockup, onNavigateToLayerStudio }) => {
    ...
    const analyzeColors = async (imgBase64: string, variation: 'NATURAL' | 'VIVID' | 'PASTEL' | 'DARK') => {
        ...
        try {
            ...
            if (data.success && data.colors) {
                ...
                // CORREÇÃO: Sempre atualiza o prompt se a API retornar um novo
    ...
    const handleGenerate = async () => {
        // CORREÇÃO: Permite gerar só com prompt de texto se não tiver imagem
        if (!referenceImage && !technicalPrompt) {
            setError("Defina um prompt ou carregue uma referência.");
            return;
        }
    ...
    return (
        <div className="h-full bg-[#f8fafc] flex flex-col overflow-hidden">
            <ModuleHeader 
                ...
                actionLabel={referenceImage || generatedPattern ? "Nova Criação" : undefined}
            ...
            {!referenceImage && !generatedPattern && !technicalPrompt ? (
                        ...
                        secondaryAction={
                            <div className="h-full flex flex-col justify-center">
                                <div className="flex items-center gap-2 mb-4">
                                    ...
                                    <h3 className="text-xs font-bold text-gray-400 uppercase tracking-widest">Opções de Criação</h3>
                                </div>
                                <button 
                                    onClick={() => { setTechnicalPrompt("Floral watercolor summer print"); }} 
                                    className="w-full py-6 border-2 border-dashed border-gray-300 rounded-xl flex flex-col items-center justify-center text-gray-400 hover:bg-white hover:border-vingi-400 transition-all gap-2 bg-gray-50 mb-4"
                                >
                                    <Wand2 size={24} className="opacity-50" />
                                    <span className="text-xs font-bold">Começar sem Imagem (Prompt Livre)</span>
                                </button>
                            </div>
                        }
                    />
                    ...
                    <div className="w-full lg:w-[400px] bg-white border-r border-gray-200 flex flex-col z-20 shadow-xl h-[40vh] lg:h-full overflow-y-auto custom-scrollbar shrink-0">
                        <div className="p-5 space-y-6 pb-20">
                            ...
                            <div className="bg-slate-50 p-4 rounded-xl border border-slate-100">
                                ...
                                {selvedgeInfo && (
                                    <div className="mb-3 p-2 bg-blue-50 border border-blue-100 rounded text-[10px] text-blue-700 flex items-start gap-2">
                                        ...
                                        <button onClick={() => setSelvedgeInfo(false)} className="ml-auto p-1 hover:bg-blue-100 rounded"><X size={12}/></button>
                                    </div>
                                )}
                            ...
                            <div>
                                ...
                                {/* HIDDEN TECHNICAL PROMPT EDITOR */}
                                <div className="mb-4">
                                     <label className="text-[9px] font-bold text-gray-400 uppercase flex justify-between items-center mb-1">
                                        Prompt Técnico (IA)
                                     </label>
                                     <textarea 
                                        value={technicalPrompt}
                                        onChange={(e) => setTechnicalPrompt(e.target.value)}
                                        className="w-full h-16 p-2 bg-slate-50 border border-slate-200 rounded-lg text-[10px] font-mono text-slate-600 resize-none outline-none"
                                     />
                                </div>
